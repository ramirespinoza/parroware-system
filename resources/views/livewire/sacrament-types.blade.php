            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                ...
                <div class="p-8 mb-6">
                    <h2 class="text-xl font-bold mb-2">
                        @if($editId)
                            Editar Tipo de Sacramento
                        @else
                            Registrar Tipo de Sacramento
                        @endif
                    </h2>
                ...
                <table class="table-auto w-full border">
                    ...
                    <tbody>
                        @foreach($sacrament_types as $sacrament_type)
                        <tr>
                            <td class="border py-3 px-6 text-left">{{ $sacrament_type->id }}</td>
                            <td class="border py-3 px-6 text-left">{{ $sacrament_type->name }}</td>
                            <td class="border py-3 px-6 text-left">{{ $sacrament_type->description }}</td>
                            <td class="border py-3 px-6 text-center">
                                <button wire:click="edit({{ $sacrament_type->id }})" class="bg-yellow-500 text-white px-2 py-1 rounded">Editar</button>
                                <button wire:click="delete({{ $sacrament_type->id }})" class="bg-red-500 text-white px-2 py-1 rounded">Eliminar</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
